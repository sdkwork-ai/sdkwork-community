...
This is the `sdkwork-community` application and workspace root. Read `sdkwork.app.config.json` for application identity, registration, SDK/API inventory, release metadata, and app-owned capabilities. Concrete environments, deployment profiles, runtime targets, bind settings, and dependency Base URLs are owned by `etc/`.
...
- `sdkwork.app.config.json`: application identity and release declaration.
...
- `specs/`: repository-wide component, topology, and IAM composition contracts.
...
- Local directories to inspect first when relevant: `apis/`, `apps/`, `crates/`, `docs/`, `packages/`, `scripts/`, `sdks/`, `tools/`.
